...
struct jsonrpc_adapter: ijsonrpc
{
  typedef std::weak_ptr<iinterface> itf_ptr_t;
  typedef std::weak_ptr<ijsonrpc> jsonrpc_ptr_t;
  
  jsonrpc_adapter() = default;
  jsonrpc_adapter(const jsonrpc_adapter&) = default;
  jsonrpc_adapter(jsonrpc_adapter&&) = default;

  jsonrpc_adapter& operator=(const jsonrpc_adapter&) = default;
  jsonrpc_adapter& operator=(jsonrpc_adapter&&) = default;
  
  jsonrpc_adapter(itf_ptr_t itf, jsonrpc_ptr_t jsonrpc)
  ...
  virtual void perform_outgoing(outgoing_holder holder, io_id_t io_id) override
  {
    ...
    {
//#error TODO: смотри engine::upgrate_options_(O& opt, ::iow::io::incoming_handler_t handler)
