  ...
  if ( holder.is_request() || holder.is_notify() )
  {
    std::vector<witf> cli;
    {
      ...
      for (const auto& m: _map)
      {
        if ( io_id != m.first )
        {
          std::cout << "################### reserver " << m.first << std::endl;
          std::cout << "################### reserver " << m.first << std::endl;
          std::cout << "################### reserver " << m.first << std::endl;
          std::cout << "################### reserver " << m.first << " входящий io_id=" << io_id<< std::endl;
    ...
    if ( !cli.empty() )
    {
      ...
      for (auto wc: cli)
      {
        if ( auto c = wc.lock() )
        {
          c->perform_io(std::make_unique<data_type>(d), this->get_id(), [](data_ptr d)
          {
            JSONRPC_LOG_WARNING("HUB prohibited message: [" << d << "]")
          });
        }
      }
    }
