...
bool variant_match::configure(const char* beg, const char* end, wjson::json_error& err)
{
...
bool variant_match::match(const char* beg, const char* end, wjson::json_error& err)
{
  if ( _matchers.empty() )
    return true;
